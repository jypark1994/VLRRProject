...
train_loader, test_loader = CUB200_MultiScale_get_loaders(args.root, batch_size=args.batch_size)
...
data_name = "CUB200_2011"

teacher_name = 'resnet34'
student_name = 'resnet34'
print(f"Teacher: {teacher_name}, Student: {student_name}")

net_t = get_model(teacher_name)
net_t = net_t.to(device)
teacher_dict = torch.load(args.teacher_weight)
print(
    f"Distillate teacher's HR(x1) knowledge ({teacher_dict['acc']*100:.2f}%) to the student.")
net_t.load_state_dict(teacher_dict['net'])

net_s = get_model(student_name)
...
def distillate(teacher, student, train_loader, cur_epoch):
    params = {'alpha': args.alpha, 'T': args.temperature}
    ...
    for i, data in enumerate(train_loader):
        ...
        writer.add_scalar("loss/KD",soft_loss,global_step=i+data_len*cur_epoch)
        writer.add_scalar("loss/CE",data_loss,global_step=i+data_len*cur_epoch)
        writer.add_scalar("loss/Total",loss,global_step=i+data_len*cur_epoch)
        ...
        writer.add_scalar("acc/train",batch_acc,global_step=i+data_len*cur_epoch)
        ...
        if(i == len(train_loader)-1):
            print(
                f"Soft loss = {soft_loss:.4f}, Hard loss = {data_loss:.4f}, Total loss = {loss:.4f}")
...
def train_and_eval(teacher, student, epochs, train_loader, test_loader, save_name='default.pth'):
    ...
    for i in range(epochs):
        ...
        train_acc, loss, student = distillate(teacher, student, train_loader, i)
